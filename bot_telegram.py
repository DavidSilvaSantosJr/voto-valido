...
def ask_location(message):
    ...
    markup.add(itembtn)
    bot.send_message(chat_id, "Por favor, envie sua localização atual, cicando no botão abaixo:", reply_markup=markup)
...
def ask_location_sem_local(message):
    ...
    bot.send_message(chat_id, "insira o nome do estado: ")
...
def save_photo(message):
    ...
    bot.send_message(chat_id, "Por favor, descreva o problema:")
    print(user_data)
...
def save_data(message):
    ...
    if toxicidade:
        bot.send_message(chat_id, "Opa! cuidado com as palavras, insira novamente a descrição. Seja cortez, ninguém gosta da falta de educação!")
    ...
    if not toxicidade:
        ...
        print('DEPOIS DE ADD NO MONGO', user_data)
        del user_data['_id'] 
...
def save_data_longe(message): 
    ...
    if toxicidade:
        user_data['state_location'] = 'sem confirmacao'
        bot.send_message(chat_id, "Opa! cuidado com as palavras, insira novamnte a descrição. Seja cortez, ninguém gosta da falta de educação!")
    
    else:
        user_data['state_location'] = 'confirmar localizacao'
        user_data['descricao'] = message.text
        bot.send_message(chat_id, 'insira o endereço do local.\n\ndiga o estado, cidade, bairro e rua. Atente-se aos detalhes!')
...
def save_data_longe(message):
        ...
        bot.send_message(chat_id, "verifique se é aqui que o problema se encontra.", reply_markup=markup)
